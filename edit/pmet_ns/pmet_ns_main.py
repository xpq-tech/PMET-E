...
from .pmet_ns_hparams import PMET_NSHyperParams
...
def apply_pmet_ns_to_model(
    model: AutoModelForCausalLM,
    tok: AutoTokenizer,
    requests: List[Dict],
    hparams: PMET_NSHyperParams,
    copy=False,
    ...
    deltas = execute_attn_memit(model, tok, requests, hparams, cache_template=cache_template) #存储了Equ14 左右的值
...
def execute_attn_memit(
    ...
    for layer in hparams.layers:
        ...
        for rewrite_module_name in rewrite_module_names:
            ...
            upd_matrix =  targets @ layer_ks.T @ torch.inverse(layer_ks @ layer_ks.T + 
