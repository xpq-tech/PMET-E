    ...
    for run_dir in (RESULTS_DIR / dir_name if not abs_path else dir_name).iterdir():
        ...
        for case_file in files:
            ...
            for prefix in ["pre", "post"]:
                ...
                for key in ["implicit_knowledge_prompts_probs", "implicit_knowledge_prompts_correct"]:
                    if prefix in data and key in data[prefix]:
                        if key == "implicit_knowledge_prompts_probs" and data[prefix][key]!= []:
                            cur_sum[f"{prefix}_avg_IK_prompt_probability"].append(np.mean(data[prefix][key]))
                        if key == "implicit_knowledge_prompts_correct" and data[prefix][key]!= []:
                            cur_sum[f"{prefix}_correct_IK_prompt"].append(data[prefix][key].count(True)/len(data[prefix][key]))
            cases += 1
        if len(cur_sum) == 0:
            continue
        ...
        for k, v in cur_sum.items():
            if all(exclude not in k for exclude in ["essence_score", "time", "post_avg_IK_prompt_probability"]):
